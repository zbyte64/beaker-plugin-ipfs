...
const setup =
exports.setup = function (directory) {
  // get a controller for the local ipfs node
  ipfsd.local(directory, (err, _ipfsNode) => {
    if (err) {
      // note the error
    ...
    if (ipfsNode.initialized)
      ...
      log.debug('[IPFS] Initializing', directory, ', keysize', KEYSIZE)
      ipfsNode.init({ keySize: KEYSIZE, directory }, (err, res) => {
        if (err) {
