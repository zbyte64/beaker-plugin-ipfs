...
exports.setup = function () {
  ...
  ipfsd.local((err, _ipfsNode) => {
    ...
    if (ipfsNode.initialized)
      ...
      log.debug('Initializing ~/.ipfs, keysize', KEYSIZE)
...
exports.lookupLink = function (folderKey, path, cb) {
  if (!ipfsApi) {
    log.warn('IPFS Daemon has not setup yet, aborting lookupLink')
    return cb({ notReady: true })
  }

  log.debug('Looking up', path, 'in', folderKey)
  var pathParts = fixPath(path).split('/')
  descend(folderKey)

  function descend (key) {
    log.debug('Listing...', key)
    ipfsApi.object.links(key, { enc: (typeof key == 'string' ? 'base58' : false) }, (err, links) => {
      if (err) return cb(err)
      
      // lookup the entry
      log.debug('folder listing for', key, links)
...
function startDaemon () {
  ...
  ipfsNode.startDaemon(function (err, _ipfsApi) {
    ...
    ipfsApi = _ipfsApi
    log.debug('Daemon active')

    // output current version
    ipfsApi.version()
      .then((res) => {
        log.debug('Using version', res.Version)
      })
      .catch((err) => {
